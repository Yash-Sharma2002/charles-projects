...
import { OrganisationAPI, WorkspaceAPI } from "../../constants/api";
...
export default function Organisation() {
  ...
  async function submit() {
    ...
    const response = await axios
      .post(OrganisationAPI + "/create", {
        organisation: {
          organisation_name: org.name,
          organisation_email: org.email,
          organisation_image: org.logo,
          organisation_website: org.website,
          organisation_type: org.type,
          organisation_phone: phoneNumber.pCode + "-" + phoneNumber.number,
        },
        uid: user.uid,
    ...
    if (response.status === ResStatus.Success) {
      ...
      setDataForOrganisation(response.data);
      await createWorkspace(response.data.organisation_id);
  ...
  async function createWorkspace(organisation_id: string) {
    const data = {
      ...
      workspace:{
        ...
        workspace_description: "Default Workspace",
        workspace_group: "Default Group"
      }
    };

    let res = await axios
      .post(WorkspaceAPI + "/create", data)
      .then((res) => res.data)
          ...
          <div className="mt-3 w-[95%] mx-auto">
            ...
            <div className="flex justify-between items-center w-full">
              ...
              <InputNumber
                defValue={""}
                inputClassName="w-[64%] "
                onChangeHandler={(e) => handleChanges("number", e.target.value)}
                name="number"
                error={
